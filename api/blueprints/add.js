 ...
 * Module dependencies
 */
var actionUtil = require('sails/lib/hooks/blueprints/actionUtil.js');
...
module.exports = function addToCollection (req, res) {
  ...
  async.auto({
    ...
    parent: function (cb) {
      var searchModel;
      if(criteria.company) {
        searchModel = Model.findOne(parentPk).where({company: criteria.company})
      } else {
        searchModel = Model.findOne(parentPk)
      }

      searchModel.exec(function foundParent(err, parentRecord) {
    ...
    actualChildPkValue: ['parent', function(parentRecord, cb) {
